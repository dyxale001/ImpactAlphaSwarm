...
class SentimentAggregator:
	"""Turns a list of scored mentions into a single signed score"""
	...
	def recency_weight(self, created_at: str | None) -> float:
		...
		more than an older one of the SAME tier"""
		if not created_at:
			return 1.0
		try:
		...
		if published.tzinfo is None:
			published = published.replace(tzinfo=timezone.utc)#UTC Time Zone
