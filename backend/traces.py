...
Usage:
    trace = Tracer(run_id="run_001", user_id="user_123")
    trace.log_step("collect_stocktwits", {"count": 10, "tickers": ["NVDA"]})
    trace.save()  # writes to ./data/runs/run_001.trace.json

Privacy & Retention:
- Anonymize user handles if required.
- Configure retention policy (e.g., keep raw messages 90 days, aggregates 1+ years).
"""
...
class SocialMention:
    ...
    source: str  # "stocktwits", "reddit", "telegram"
    message_id: str
    raw_text: str
    cleaned_text: str
    created_at: str
...
class Tracer:
    ...
    Saves JSON snapshots to local disk.
    """
    ...
    def __init__(
        ...
        self.trace: Dict[str, Any] = {
            ...
            "steps": [],
            "collectors": {
                "messages": [],  # List of SocialMention dicts
                "quant_metrics": {},  # {ticker: QuantMetrics dict}
            },
            "sentiment_outputs": {},  # {ticker: {"sentiment_score", "bullish_posts", ...}}
            "aggregates": {},  # Final scores, penalties, reasoning
            "artifacts": {
                "trace_path": None,
                "saved_at": None,
            },
    ...
    def log_step(self, step_name: str, metadata: Optional[Dict[str, Any]] = None):
        """
        Log a major orchestrator step.
        
        Args:
            step_name: e.g., "collect_stocktwits", "score_mentions", "synthesize"
            metadata: step-specific data (e.g., counts, sample top_posts)
        """
        step_entry = {
            "step": step_name,
    ...
    def add_messages(self, ticker: str, messages: List[SocialMention]):
        """Add collected social media messages to the trace."""
        for msg in messages:
    ...
    def add_quant_metrics(self, ticker: str, metrics: QuantMetrics):
        """Add quantitative metrics to the trace."""
    ...
    def add_sentiment_output(self, ticker: str, sentiment_data: Dict[str, Any]):
        """Add sentiment analysis results to the trace."""
    ...
    def add_aggregates(
        ...
        """Add final aggregates and reasoning to the trace."""
...
def list_traces(limit: int = 10) -> List[str]:
    """List recent saved traces (up to `limit`)."""
