...
export default function WhaleWatching({ ticker }: { ticker: string }) {
  ...
  // Distinct transaction types present, so the legend explains each type once
  // rather than repeating a tooltip on every (often duplicated) row.
  ...
  return (
    <section className="soft-card w-full p-5 space-y-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="shrink-0 inline-flex items-center gap-1.5 rounded-full border border-brand-border/60 bg-brand-bg/55 px-3 py-1.5 text-xs font-semibold text-brand-muted-fg">
